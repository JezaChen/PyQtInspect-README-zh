...
- [**分离模式**](#33-分离模式启动-传统方法-一对多调试): 先手动启动GUI服务端, 再启动被调试端以连接服务端. **被调试端关闭后, GUI服务端不会关闭.**

- [**直接模式 (推荐)**](#32-直接模式启动-便捷方法-推荐): 仅需启动被调试端, 被调试端会自行**在本机启动**一个GUI服务端(无须开发者事先手动启动服务端). **被调试端关闭后会一同关闭GUI服务端.**
...
此外, PyQtInspect还支持[在PyCharm等IDE上运行](#341-在pycharm等ide上运行pyqtinspect-支持分离模式直接模式),
以及[通过Attach进程的方式附加到PyQt/PySide进程中进行调试](#342-attach进程仅支持分离模式-目前不稳定).
...
#### 3.4.1 在PyCharm等IDE上运行PyQtInspect (支持分离模式/直接模式)
...
**p.s. 对于通过[Attach进程方式](#342-attach进程仅支持分离模式-目前不稳定)启动的PyQtInspect客户端, 如果Attach过程中控件已经创建好了, 此时是拿不到创建时的信息, 调用栈信息为空**
